...
class PLAYER(pygame.sprite.Sprite):
    ...
    def __init__(self, x_loc, y_loc):
        ...
        pygame.draw.circle(self.image, WHITE, (PLAYER_CENTER, PLAYER_CENTER), PLAYER_RADIUS)
    ...
    def movement(self, d_x, d_y, unitx, unity):
        ...
        pygame.draw.circle(self.image, WHITE, (PLAYER_CENTER, PLAYER_CENTER), PLAYER_RADIUS)
        pygame.draw.line(self.image, WHITE, [PLAYER_CENTER, PLAYER_CENTER], [PLAYER_CENTER * unitx + PLAYER_CENTER, PLAYER_CENTER * unity + PLAYER_CENTER], PLAYER_RADIUS // 2)
    ...
    def shoot(self, unitx, unity):
        ...
        pellet = Bullet(self.rect.x + PLAYER_CENTER + (2 * PLAYER_RADIUS * unitx), self.rect.y + PLAYER_CENTER + (2 * PLAYER_RADIUS * unity), unitx, unity, SLOW_BULLET)
        GUN.add(pellet)
...
class Zombie(pygame.sprite.Sprite):
    ...
    def __init__(self):
        super().__init__()
        #Draw the zombie
        self.image = pygame.Surface((ZOMBIE_SIZE, ZOMBIE_SIZE), pygame.SRCALPHA)
        pygame.draw.circle(self.image, GREEN, [ZOMBIE_RADIUS, ZOMBIE_RADIUS], ZOMBIE_RADIUS)
        pygame.draw.ellipse(self.image, RED, [0, 0, ZOMBIE_SIZE, ZOMBIE_SIZE], 2)

        self.rect = self.image.get_rect()

        #set the spawn locaiton
        spawn_loc = random.randint(1, 4)
        if spawn_loc == 1:
        ...
        self.speed = random.randint(SLOW_ZOMB, FAST_ZOMB)
        ENEMIES.add(self)
...
class Bullet(pygame.sprite.Sprite):
    ...
    def __init__(self, x_loc, y_loc, unitvx, unitvy, speed): #coordinates and direction of bullet and fast or slow bullet
        super().__init__()
        #draw the bullet
        self.image = pygame.Surface((BULLET_BOX, BULLET_BOX), pygame.SRCALPHA)
        pygame.draw.circle(self.image, WHITE, [BULLET_RADIUS, BULLET_RADIUS], BULLET_RADIUS)
...
while not DONE:
    ...
    MAG = math.hypot(VECTORX, VECTORY)
    ...
    for zom in ENEMIES:
        ...
        if pygame.sprite.collide_circle(PLAYER, zom):
            ...
            DONE = True
            break
        zom.move_towards_player(PLAYER.rect.x + PLAYER_CENTER, PLAYER.rect.y + PLAYER_CENTER)
    ...
    #setup score display
    SCORE_IMAGE = pygame.font.SysFont("arial", 30).render("Score: %i"% SCORE, True, WHITE)
